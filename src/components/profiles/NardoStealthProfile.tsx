            ...
            {buttons.map((btn, i) => {
              ...
              return (
                <motion.a
                  ...
                  className="flex flex-col items-center gap-2 p-3 rounded-xl bg-white/[0.04] backdrop-blur border border-nardo/15 text-slate-light hover:border-nardo/40 hover:bg-white/[0.08] hover:shadow-[0_0_15px_rgba(192,192,192,0.1)] transition-all duration-300 group"
                  initial={{ opacity: 0, y: 20 }}
                  animate={{ opacity: 1, y: 0 }}
                ...
                >
                  <div className="w-10 h-10 rounded-full bg-nardo/10 flex items-center justify-center group-hover:bg-nardo/20 transition-colors">
                    <Icon className="w-5 h-5 text-nardo" />
                  </div>
                  <span className="text-center text-xs font-medium text-slate-muted group-hover:text-slate-light">{btn.label}</span>
                </motion.a>
              );
            })}
          </div>
